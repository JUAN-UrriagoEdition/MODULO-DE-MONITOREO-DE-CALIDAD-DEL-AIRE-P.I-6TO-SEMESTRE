/*
 * ============================================================
 *  P.I6Def — Monitor de Gases + WiFi HTTP Server
 *  ESP32-S3-WROOM-1
 *
 *  Endpoint: GET http://<IP_ESP32>/datos
 *  Responde JSON:
 *  {
 *    "co":  85.3,
 *    "no2": 0.42,
 ...
 *  }
 *
 *  Estados: REPOSO | ADVERTENCIA | PELIGRO | ALARMA
 *
 *  Mapa de pines:
 *    SDA_CCS811 → GPIO39
 *    SCL_CCS811 → GPIO38
 ...
 *    SEN0574 NO2 → GPIO18
 *
 *  Librerías: SparkFun CCS811, WiFi (built-in ESP32)
 * ============================================================
 */
...
// ─── WiFi ─────────────────────────────────────────────────
#define WIFI_SSID   "Familia Urriago"       // ← cambia esto
#define WIFI_PASS   "3113506859"     // ← cambia esto
...
// ─── Diagrama de estados ───────────────────────────────────
typedef enum { REPOSO, ADVERTENCIA, PELIGRO, ALARMA } Estado;
Estado estadoActual = REPOSO;
unsigned long tiempoPeligro = 0;      // millis desde que entró en PELIGRO
#define TIEMPO_ALARMA_MS  30000       // 30s en PELIGRO → ALARMA
...
//  CONVERSIONES
// ══════════════════════════════════════════════════════════
float rawAV(int raw)       { return (raw / ADC_MAX) * VREF; }
float vACO(float v)        { return v <= 0 ? 0 : (v / V_CO_REF) * CO_REF_PPM; }
float vANO2(float v)       { float d = V0_NO2 - v; return d < 0 ? 0 : d / FACTOR_NO2; }

// ══════════════════════════════════════════════════════════
//  DIAGRAMA DE ESTADOS
// ══════════════════════════════════════════════════════════
bool enPeligro(float co, float no2, float co2, float voc) {
  return co >= U_CO_PEL || no2 >= U_NO2_PEL ||
         co2 >= U_CO2_PEL || voc >= U_VOC_PEL;
}
bool enAdvertencia(float co, float no2, float co2, float voc) {
  return co >= U_CO_ADV || no2 >= U_NO2_ADV ||
         co2 >= U_CO2_ADV || voc >= U_VOC_ADV;
}
int gasesEnPeligro(float co, float no2, float co2, float voc) {
  return (co  >= U_CO_PEL  ? 1 : 0) + (no2 >= U_NO2_PEL ? 1 : 0) +
         (co2 >= U_CO2_PEL ? 1 : 0) + (voc >= U_VOC_PEL ? 1 : 0);
}
...
void actualizarEstado(float co, float no2, float co2, float voc) {
  ...
  switch (estadoActual) {
    ...
    case REPOSO:
      if (pel)       { estadoActual = PELIGRO;     tiempoPeligro = millis(); }
      else if (adv)  { estadoActual = ADVERTENCIA; }
      break;

    case ADVERTENCIA:
      if (pel)       { estadoActual = PELIGRO;     tiempoPeligro = millis(); }
      else if (!adv) { estadoActual = REPOSO; }
      break;
    ...
    case ALARMA:
      if (!pel && !adv) { estadoActual = REPOSO; tiempoPeligro = 0; }
      else if (!pel)    { estadoActual = ADVERTENCIA; tiempoPeligro = 0; }
      break;
  }
}
